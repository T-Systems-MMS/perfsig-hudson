...
public class DTPerfSigProjectAction implements ProminentProjectAction {
    ...
    @SuppressWarnings("unused")
    public void doGetDashboardConfiguration(final StaplerRequest request, StaplerResponse response) throws IOException {
        final String testCase = request.getParameter("testCase");
        ...
        try {
            response.serveFile(request, fileInputStream, input.lastModified(), (int) input.length(), "mime-type:application/json");
	...
	@SuppressWarnings("unused")
    public void doSetDashboardConfiguration(final StaplerRequest request, StaplerResponse response) throws IOException {
        final String testCase = request.getParameter("testCase");
        if (StringUtils.isBlank(testCase)) return;
    ...
    @SuppressWarnings("unused")
    public void doGetAvailableMeasures(final StaplerRequest request, StaplerResponse response) throws IOException {
        final String dashlet = request.getParameter("dashlet");
        if (StringUtils.isBlank(dashlet)) return;
        ...
        JSONArray json = JSONArray.fromObject(availableMeasures);
	...
	private String getDashboardConfiguration(String testCase) {
        if (StringUtils.isBlank(testCase)) return "";
        File input = new File(getJsonConfigFilePath() + File.separator + testCase + "-config.json");
        if (!input.exists()) {
            input = new File(DTPerfSigUtils.getInstanceOrDie().getRootDir() + "/plugins/" + Messages.DTPerfSigProjectAction_UrlName() + "/defaultConfig.json");
