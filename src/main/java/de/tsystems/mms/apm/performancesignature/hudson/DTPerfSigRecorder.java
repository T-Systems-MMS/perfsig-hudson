...
import de.tsystems.mms.apm.performancesignature.hudson.model.ConfigurationTestCase.ConfigurationTestCaseDescriptor;
import de.tsystems.mms.apm.performancesignature.hudson.model.Dashboard;
import de.tsystems.mms.apm.performancesignature.hudson.model.GeneralTestCase;
...
public class DTPerfSigRecorder extends Recorder {
    private final String protocol, host, username, password, profile;
    ...
    public DTPerfSigRecorder(final String protocol, final String host, final int port, final String username, final String password,
                             final boolean verifyCertificate, final String profile, final boolean exportSessions, final int delay, final int retryCount,
    ...
    public boolean perform(final AbstractBuild build, final Launcher launcher, final BuildListener listener) throws InterruptedException, IOException {
        ...
        final DTServerConnection connection = new DTServerConnection(this.getProtocol(), this.getHost(), this.getPort(), this.getUsername(),
                this.getPassword(), verifyCertificate, useJenkinsProxy, customProxy);
...
public String getUsername() {
    ...
    public static final class DescriptorImpl extends BuildStepDescriptor<Publisher> {
        ...
        public FormValidation doCheckUsername(@QueryParameter final String username) {
            FormValidation validationResult;
            if (checkNotNullOrEmpty(username)) {
                validationResult = FormValidation.ok();
            } else {
        ...
        public FormValidation doCheckPassword(@QueryParameter final String password) {
            FormValidation validationResult;
            if (checkNotNullOrEmpty(password)) {
                validationResult = FormValidation.ok();
            } else {
                validationResult = FormValidation.error(Messages.DTPerfSigRecorder_DTPasswordEmpty());
            }
            return validationResult;
        }
        ...
        public FormValidation doTestDynaTraceConnection(@QueryParameter("protocol") final String protocol, @QueryParameter("host") final String host,
                                                        @QueryParameter("port") final int port, @QueryParameter("username") final String username,
                                                        @QueryParameter("password") final String password, @QueryParameter("verifyCertificate") final boolean verifyCertificate,
                                                        @QueryParameter("useJenkinsProxy") final boolean useJenkinsProxy,
            ...
            final DTServerConnection connection = new DTServerConnection(protocol, host, port, username, password, verifyCertificate, useJenkinsProxy, proxy);
